package asl
 ...
 class artillery  (coms: AgentCommunicationLayer = new  DefaultCommunications,
         ...
         message match {
            case SubGoalMessage(_,_,r) =>
               message.goal match {
                   ...
                   case artillery.this.adopt_belief_targetposition_2 =>
                     artillery.this.adopt_belief_targetposition_2.execute(message.params.asInstanceOf[Parameters])
 ...
 class Agent extends IAgent {
         ...
         override def agent_type: String = "artillery"
   ...
   object GoalParser extends IAgentGoalParser {
        ...
        override def create_belief_message(t: StructTerm, ref: IMessageSource) (implicit executionContext: ExecutionContext): Option[SubGoalMessage] = {
                                   if(t.matchOnlyFunctorAndArity("targetposition",2)) {
                                     val args: Parameters = Parameters(t.terms.toList)
                                     Option(SubGoalMessage(adopt_belief_targetposition_2, args, ref))
                                   } else   {
                    Option.empty[SubGoalMessage]
                    }
      ...
      object adopt_belief_targetposition_2 extends IGoal {
         ...
         vars("Parent").bind_to(StringTerm(executionContext.parent.name))
             ...
             executionContext.src.asInstanceOf[AkkaMessageSource].address() ! IntentionErrorMessage(NoApplicablePlanMessage(),AkkaMessageSource(executionContext.agent.self))
                      ...
                      def plan0(vars: VarMap)(implicit executionContext: ExecutionContext): Unit = {
                                           ...
                                           BeliefUpdateAction.execute(BeliefUpdateAction.Parameters("+", StructTerm("fired",Seq[GenericTerm](vars("X"),vars("Y")))),GoalParser)
                                          PrimitiveAction.execute(PrimitiveAction.Parameters(() => coms.inform(StructTerm("environment",Seq[GenericTerm]()),StructTerm("fired",Seq[GenericTerm](vars("X"),vars("Y"))))))
                                          PrimitiveAction.execute(PrimitiveAction.Parameters(() => println( ( ( (StringTerm("Fire at ") + vars("X"))  + StringTerm(", "))  + vars("Y")) )))
...
object artillery_companion { 
   def create() = new artillery().agentBuilder 
   def create(in_coms : AgentCommunicationLayer) = new artillery(coms = in_coms).agentBuilder 
   def create(in_beliefBaseFactory: IBeliefBaseFactory) = new artillery(beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
   def create(in_coms : AgentCommunicationLayer, in_beliefBaseFactory: IBeliefBaseFactory) = new artillery(coms = in_coms, beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
} 
