package asl
 ...
 class droneB  (coms: AgentCommunicationLayer = new  DefaultCommunications,
         ...
         message match {
            case SubGoalMessage(_,_,r) =>
               message.goal match {
                   ...
                   case droneB.this.adopt_achievement_start_0 =>
                     droneB.this.adopt_achievement_start_0.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_belief_antidronefire_2 =>
                     droneB.this.adopt_belief_antidronefire_2.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_achievement_radar_0 =>
                     droneB.this.adopt_achievement_radar_0.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_belief_targeting_3 =>
                     droneB.this.adopt_belief_targeting_3.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_achievement_moveup_0 =>
                     droneB.this.adopt_achievement_moveup_0.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_achievement_movedown_0 =>
                     droneB.this.adopt_achievement_movedown_0.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_achievement_moveleft_0 =>
                     droneB.this.adopt_achievement_moveleft_0.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_achievement_moveright_0 =>
                     droneB.this.adopt_achievement_moveright_0.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_achievement_whereIam_2 =>
                     droneB.this.adopt_achievement_whereIam_2.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_achievement_target_2 =>
                     droneB.this.adopt_achievement_target_2.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_belief_attacked_1 =>
                     droneB.this.adopt_belief_attacked_1.execute(message.params.asInstanceOf[Parameters])

                   case droneB.this.adopt_achievement_additionaltasks_0 =>
                     droneB.this.adopt_achievement_additionaltasks_0.execute(message.params.asInstanceOf[Parameters])
 ...
 class Agent extends IAgent {
         ...
         override def agent_type: String = "droneB"
   ...
   object GoalParser extends IAgentGoalParser {
        override def create_goal_message(t: StructTerm, ref: IMessageSource) (implicit executionContext: ExecutionContext): Option[SubGoalMessage] = {
                                   ...
                                   if(t.matchOnlyFunctorAndArity("moveright",0)) {
                                     val args: Parameters = Parameters(t.terms.toList)
                                     Option(SubGoalMessage(adopt_achievement_moveright_0, args, ref))
                                   } else  
      ...
      object adopt_achievement_moveright_0 extends IGoal {
         ...
         vars("Parent").bind_to(StringTerm(executionContext.parent.name))
             ...
             executionContext.src.asInstanceOf[AkkaMessageSource].address() ! IntentionErrorMessage(NoApplicablePlanMessage(),AkkaMessageSource(executionContext.agent.self))
                      ...
                      def plan0(vars: VarMap)(implicit executionContext: ExecutionContext): Unit = {
                                          ...
                                          adopt_achievement_moveright_0.execute(Parameters(List(  )))
                                          adopt_achievement_moveright_0.execute(Parameters(List(  )))
                                          adopt_achievement_radar_0.execute(Parameters(List(  )))
...
object droneB_companion { 
   def create() = new droneB().agentBuilder 
   def create(in_coms : AgentCommunicationLayer) = new droneB(coms = in_coms).agentBuilder 
   def create(in_beliefBaseFactory: IBeliefBaseFactory) = new droneB(beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
   def create(in_coms : AgentCommunicationLayer, in_beliefBaseFactory: IBeliefBaseFactory) = new droneB(coms = in_coms, beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
} 
