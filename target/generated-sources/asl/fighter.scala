package asl
 ...
 class fighter  (coms: AgentCommunicationLayer = new  DefaultCommunications,
         ...
         message match {
            case SubGoalMessage(_,_,r) =>
               message.goal match {
                   ...
                   case fighter.this.adopt_belief_dronelocation_3 =>
                     fighter.this.adopt_belief_dronelocation_3.execute(message.params.asInstanceOf[Parameters])

                   case fighter.this.adopt_achievement_firemissile_3 =>
                     fighter.this.adopt_achievement_firemissile_3.execute(message.params.asInstanceOf[Parameters])
 ...
 class Agent extends IAgent {
         ...
         override def agent_type: String = "fighter"
         ...
         def initBeliefs()(implicit executionContext: ExecutionContext) = List[StructTerm](
                     StructTerm("missiles",Seq[GenericTerm](IntTerm(2)))
   ...
   object GoalParser extends IAgentGoalParser {
        override def create_goal_message(t: StructTerm, ref: IMessageSource) (implicit executionContext: ExecutionContext): Option[SubGoalMessage] = {
                                   ...
                                   if(t.matchOnlyFunctorAndArity("firemissile",3)) {
                                     val args: Parameters = Parameters(t.terms.toList)
                                     Option(SubGoalMessage(adopt_achievement_firemissile_3, args, ref))
                                   } else   {
        ...
        override def create_belief_message(t: StructTerm, ref: IMessageSource) (implicit executionContext: ExecutionContext): Option[SubGoalMessage] = {
                                   if(t.matchOnlyFunctorAndArity("dronelocation",3)) {
                                     val args: Parameters = Parameters(t.terms.toList)
                                     Option(SubGoalMessage(adopt_belief_dronelocation_3, args, ref))
                                   } else    {
                    Option.empty[SubGoalMessage]
                    }
      ...
      object adopt_belief_dronelocation_3 extends IGoal {
         ...
         vars("Parent").bind_to(StringTerm(executionContext.parent.name))
             ...
             executionContext.src.asInstanceOf[AkkaMessageSource].address() ! IntentionErrorMessage(NoApplicablePlanMessage(),AkkaMessageSource(executionContext.agent.self))
                      ...
                      def plan0(vars: VarMap)(implicit executionContext: ExecutionContext): Unit = {
                                          ...
                                          PrimitiveAction.execute(PrimitiveAction.Parameters(() => println( (StringTerm("fighter knows location of ") + vars("D")) )))
                                          adopt_achievement_firemissile_3.execute(Parameters(List( vars("D") , vars("X") , vars("Y")  )))
      ...
      object adopt_achievement_firemissile_3 extends IGoal {
        ...
        def execute(params: Parameters) (implicit executionContext: ExecutionContext) : Unit = {
         ...
         vars("Parent").bind_to(StringTerm(executionContext.parent.name))
                 ...
                 //plan 0 start
                         ...
                         val r0 = executionContext.beliefBase.query(StructTerm(",",Seq[GenericTerm](StructTerm(",",Seq[GenericTerm](StructTerm("missiles",Seq[GenericTerm](vars("N"))),StructTerm("is",Seq[GenericTerm](vars("NN"),StructTerm("-",Seq[GenericTerm](vars("N"),IntTerm(1))))))),StructTerm(">",Seq[GenericTerm](vars("N"),IntTerm(0))))))
                 ...
                 //plan 1 start
                         ...
                         val r1 = executionContext.beliefBase.query(StructTerm(",",Seq[GenericTerm](StructTerm("missiles",Seq[GenericTerm](vars("N"))),StructTerm("==",Seq[GenericTerm](vars("N"),IntTerm(0))))))
             ...
             executionContext.src.asInstanceOf[AkkaMessageSource].address() ! IntentionErrorMessage(NoApplicablePlanMessage(),AkkaMessageSource(executionContext.agent.self))
                      ...
                      def plan0(vars: VarMap)(implicit executionContext: ExecutionContext): Unit = {
                                           ...
                                           BeliefUpdateAction.execute(BeliefUpdateAction.Parameters("-", StructTerm("missiles",Seq[GenericTerm](vars("N")))),GoalParser)
                                           BeliefUpdateAction.execute(BeliefUpdateAction.Parameters("+", StructTerm("missiles",Seq[GenericTerm](vars("NN")))),GoalParser)
                                          PrimitiveAction.execute(PrimitiveAction.Parameters(() => println( ( ( ( ( ( (vars("Self") + StringTerm(" firing anti drone Missile at "))  + vars("X"))  + StringTerm(", "))  + vars("Y"))  + StringTerm(" to destroy "))  + vars("D")) )))
                                          PrimitiveAction.execute(PrimitiveAction.Parameters(() => println(StringTerm("missile is flying"))))
                                          PrimitiveAction.execute(PrimitiveAction.Parameters(() => coms.inform(vars("D"),StructTerm("antidronefire",Seq[GenericTerm](vars("X"),vars("Y"))))))
                                          PrimitiveAction.execute(PrimitiveAction.Parameters(() => println( (vars("NN") + StringTerm(" missiles remains")) )))


                     }
                      def plan1(vars: VarMap)(implicit executionContext: ExecutionContext): Unit = {

                                          PrimitiveAction.execute(PrimitiveAction.Parameters(() => println( ( ( ( ( ( (vars("Self") + StringTerm(" cannot fire at "))  + vars("X"))  + StringTerm(", "))  + vars("Y"))  + StringTerm(" to destroy "))  + vars("D")) )))
                                          PrimitiveAction.execute(PrimitiveAction.Parameters(() => println( (vars("Self") + StringTerm(" out of missiles")) )))
...
object fighter_companion { 
   def create() = new fighter().agentBuilder 
   def create(in_coms : AgentCommunicationLayer) = new fighter(coms = in_coms).agentBuilder 
   def create(in_beliefBaseFactory: IBeliefBaseFactory) = new fighter(beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
   def create(in_coms : AgentCommunicationLayer, in_beliefBaseFactory: IBeliefBaseFactory) = new fighter(coms = in_coms, beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
} 
