package asl_compiler
 ...
 class poonger  (coms: AgentCommunicationLayer = new  DefaultCommunications,
         ...
         message match {
            case SubGoalMessage(_,_,r) =>
               message.goal match {
                   ...
                   case poonger.this.adopt_belief_ping_0 =>
                     poonger.this.adopt_belief_ping_0.execute(message.params.asInstanceOf[Parameters])
 ...
 class Agent extends IAgent {
         ...
         override def agent_type: String = "poonger"
...
object poonger_companion { 
   def create() = new poonger().agentBuilder 
   def create(in_coms : AgentCommunicationLayer) = new poonger(coms = in_coms).agentBuilder 
   def create(in_beliefBaseFactory: IBeliefBaseFactory) = new poonger(beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
   def create(in_coms : AgentCommunicationLayer, in_beliefBaseFactory: IBeliefBaseFactory) = new poonger(coms = in_coms, beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
} 
