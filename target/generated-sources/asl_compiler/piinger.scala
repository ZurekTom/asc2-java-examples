package asl_compiler
 ...
 class piinger  (coms: AgentCommunicationLayer = new  DefaultCommunications,
         ...
         message match {
            case SubGoalMessage(_,_,r) =>
               message.goal match {
                   ...
                   case piinger.this.adopt_achievement_init_0 =>
                     piinger.this.adopt_achievement_init_0.execute(message.params.asInstanceOf[Parameters])

                   case piinger.this.adopt_belief_pong_0 =>
                     piinger.this.adopt_belief_pong_0.execute(message.params.asInstanceOf[Parameters])
 ...
 class Agent extends IAgent {
         ...
         override def agent_type: String = "piinger"
...
object piinger_companion { 
   def create() = new piinger().agentBuilder 
   def create(in_coms : AgentCommunicationLayer) = new piinger(coms = in_coms).agentBuilder 
   def create(in_beliefBaseFactory: IBeliefBaseFactory) = new piinger(beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
   def create(in_coms : AgentCommunicationLayer, in_beliefBaseFactory: IBeliefBaseFactory) = new piinger(coms = in_coms, beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
} 
